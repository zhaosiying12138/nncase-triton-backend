...
Qwen3 note: the small autoregressive Qwen3 PoC currently uses generated paged-attention collectives, so the dense flash-attention Fusion rule is covered by a dedicated pass/lowering unit test rather than this paged-attention graph. The current Qwen3 metadata also does not contain a partial-input `elementwise.add`, so `_nncase_partial_add_rank4_kernel` is covered by the source contract and a focused CUDA micro-test instead of by the Qwen3 end-to-end graph.
