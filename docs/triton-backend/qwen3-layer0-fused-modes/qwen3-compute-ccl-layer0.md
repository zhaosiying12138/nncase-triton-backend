...
| 3 | `gather`<br>`ccl_tail.gather.grs` | Qwen3Model.embed_tokens | in: buffer_175 ids_b: i64[S] SBP=(B)<br>param: const_174 embed_w: f16[V,1024] SBP=(B,S(0))<br>out: buffer_176 embed_b_s: f16[S,1024] SBP=(B,S(0)) | `_nncase_pe_gather_axis0_rank2_kernel` |
| 4 | `all-to-all reshard`<br>`gather_reduce_scatter`<br><em>elided by producer ord3</em> | Qwen3Model.embed_tokens | in: buffer_176 embed_b_s: f16[S,1024] SBP=(B,S(0))<br>out: buffer_177 embed_s_b: f16[S,1024] SBP=(S(0),B) | `elided; ccl_tail after ord3` |
...
No `fusion.cuda.*` ops are present in this `compute-ccl` graph.
